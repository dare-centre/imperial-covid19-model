...
parameters {
  ...
  vector[3] alpha_lifted; 
  real<lower=0> gamma;
  real<lower=0> gamma_lifted;
  vector[M] lockdown;
  vector[M] lockdown_lifted;
...
transformed parameters {
  ...
  
  {
    ...
    for (m in 1:M){
      ...
      Rt[,m] = mu[m] * exp(-X[m][,1:6] * alpha
                           -X[m][,8:10] * alpha_lifted 
                           - X[m][,5] * lockdown[m]
                           - X[m][,10] * lockdown_lifted[m]
...
model {
  ...
  gamma ~ normal(0,.2);
  gamma_lifted ~ normal(0,.2);
  lockdown ~ normal(0,gamma);
  lockdown_lifted ~ normal(0,gamma_lifted);
  last_intervention ~ normal(0,gamma);
  phi ~ normal(0,5);
  kappa ~ normal(0,0.5);
  mu ~ normal(3.28, kappa); // citation: https://academic.oup.com/jtm/article/27/2/taaa021/5735319
  alpha_hier ~ gamma(.1667,1);
  alpha_lifted ~ normal(0,5);
