...
process_covariates <- function(countries, interventions, d, ifr.by.country,N2){
  ...
  x1 <- rgammaAlt(5e7,mean1,cv1) # infection-to-onset distribution
  x2 <- rgammaAlt(5e7,mean2,cv2) # onset-to-death distribution
  ...
  for (Country in countries$Regions){
    ...
    print(sprintf("First non-zero cases is on day %d, and 10 days before 10 deaths is day %d",index,index2))
